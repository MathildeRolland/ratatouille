...
it('should return a summary when the guest has choosen every meal', () => {
	const ordering: OrderingDomainModel.State = {
		...
		availableTables: {
			status: 'idle',
			error: null,
			data: [TableFactory.create({ id: 'table-1', title: 'Ma table' })],
		},
		availableMeals: {
			status: 'idle',
			error: null,
			data: [
				MealFactory.create({ id: 'meal-1', title: 'Meal 1' }),
				MealFactory.create({ id: 'meal-2', title: 'Meal 2' }),
				MealFactory.create({ id: 'meal-3', title: 'Meal 3' }),
				MealFactory.create({ id: 'meal-4', title: 'Meal 4' }),
			],
		},
	...
	expect(selectSummary(state)).toEqual({
		table: { id: 'table-1', title: 'Ma table' },
		guests: [
			{
				id: 'user-1',
				name: 'John Doe',
				isOrganizer: true,
				meals: {
					entry: { id: 'meal-1', title: 'Meal 1' },
					mainCourse: { id: 'meal-2', title: 'Meal 2' },
					dessert: { id: 'meal-3', title: 'Meal 3' },
					drink: { id: 'meal-4', title: 'Meal 4' },
				},
			},
		],
	});
...
it('should return a summary when the guest has only choosen a main course', () => {
	const ordering: OrderingDomainModel.State = {
		...
		availableMeals: {
			status: 'idle',
			error: null,
			data: [MealFactory.create({ id: 'meal-1', title: 'Meal 1' })],
		},
		reservation: { status: 'idle' },
	};
		...
		guests: [
			{
				...
				meals: {
					entry: null,
					mainCourse: { id: 'meal-1', title: 'Meal 1' },
					dessert: null,
					drink: null,
				},
			},
